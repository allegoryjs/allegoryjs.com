...
export function BlogTableOfContents({ posts }: BlogTableOfContentsProps) {
  ...
  return (
    <>
      ...
      <div className="lg:hidden mb-8">
        <Collapsible open={isOpen} onOpenChange={setIsOpen}>
          ...
          <CollapsibleContent className="overflow-hidden data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0">
            <nav
              className="mt-2 px-4 py-3 bg-card border border-border rounded-lg"
      ...
      {/* Desktop: Sticky sidebar in left margin */}
      <aside
        className="hidden lg:block fixed left-4 xl:left-8 top-24 w-64 max-h-[calc(100vh-8rem)] overflow-y-auto"
        aria-label="Table of Contents"
      >
        <nav className="p-4 bg-card border border-border rounded-lg shadow-sm">
          <h2 className="font-serif text-lg mb-4 text-foreground">
            Table of Contents
          </h2>
          <ul className="space-y-1 divide-y divide-border">
            {posts.map((post) => (
